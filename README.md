...
- **Repetições aninhadas (`repetido` dentro de `repetido`)**: cada nível de repetição precisa de índice de seção (`idx`) e contexto (`ctx`) próprios, propagados corretamente por `js/render-tree.js` (renderização/checks/contas) e por `js/speech.js` (fila de fala, `historicoRepeticoes`, `ancestraisBloco`). Foram identificados e corrigidos, nessa área, bugs de marcadores de progresso não resetados visualmente e de leitura em voz alta marcando uma repetição inteira como concluída/pulando voltas restantes prematuramente (ver oração "Coroa das Lágrimas", que tem uma repetição de 7 voltas contendo outra repetição de 7 voltas dentro). Qualquer mudança futura nesses dois arquivos que toque índices de seção ou fila de fala deveria ser testada manualmente com um caso real de repetição aninhada, não só com repetição simples de um nível.
- **Geração de `.zip` sem biblioteca externa (`criarZip` em `js/admin.js`)**: por ser uma implementação mínima e manual do formato ZIP, qualquer campo do cabeçalho que dependa de metadados do arquivo (data/hora, atributos, compressão) precisa ser calculado explicitamente — não vem "de graça" como viria usando uma biblioteca como JSZip. Já foi corrigido um caso em que a data/hora de todo arquivo dentro do zip ficava fixa em 1980-01-01 em vez da data/hora real de geração; vale ficar atento a esse tipo de lacuna ao estender essa função no futuro (ex: se um dia for preciso adicionar compressão de verdade, ou suporte a mais metadados).
